    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= 4; i++) {
            for (int j = 2; j <= 20; j++) {
                ...
                try {
                    if(getData(String.format("union-find-forest/%dto%d.in", i, j), input)) {
                        CHM(input, forest, output);
                        outData(String.format("union-find-forest/%dto%d.kirson.out", i, j), output);
                    }
                    else if(getData(String.format("%d.in", i), input) && key!=i){
                        CHM(input, forest, output);
                        outData(String.format("union-find-forest/%d.kirson.out", i), output);
                        key++;
                    }
                } catch (Exception e) {
                    e.printStackTrace();
