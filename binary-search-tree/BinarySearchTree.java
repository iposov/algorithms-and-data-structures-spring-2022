...
public class BinarySearchTree {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= 4; i++) {
            ...
            try {
                getData(String.format("binary-search-tree/%d.in", i), input);
                split(input, tree);
                createBinaryTree(tree, output_contains, output_min_after);
                outData_contains(String.format("binary-search-tree/%d.kirson.contains.out", i), output_contains);
                outData_min_after(String.format("binary-search-tree/%d.kirson.min-after.out", i), output_contains, output_min_after);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
